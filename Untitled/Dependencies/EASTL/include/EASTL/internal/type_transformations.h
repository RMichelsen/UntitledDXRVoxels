...
namespace eastl
{
	...
	#define EASTL_TYPE_TRAIT_aligned_storage_CONFORMANCE 1    // aligned_storage is conforming.
		...
		template<size_t N, size_t RoundUp = EASTL_ALIGN_OF(double)>
		struct aligned_storage
		{
			struct type { unsigned char mCharData[N]; } EA_ALIGN(RoundUp);
		};
		...
		{
			typedef struct {
				alignas(RoundUp) unsigned char mCharData[N];
			} type;
		};
		...
		template<size_t N, size_t RoundUp>
		...
		struct aligned_storage
		{
			typedef typename aligned_storage_helper<N, RoundUp>::type type;
		};
		...
		struct aligned_storage
		{
			union type
			{
				...
				struct EA_ALIGN(RoundUp) mStruct{ }; 
			};
		};
	#endif
	...
	#else
		template <size_t N, size_t RoundUp = EASTL_ALIGN_OF(double)>
		using aligned_storage_t = typename aligned_storage<N, RoundUp>::type;
